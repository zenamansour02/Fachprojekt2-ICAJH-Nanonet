...
class ICAJH13(Topo):
    def build(self):
        ...
        # Gateway ingress (100 Mbps/unit; (1→4)=200 Mbps, others=100 Mbps; w=4 HeurOSPF)
        self.add_link_name("0", "4", cost=4000, delay=0.2, bw=100000, directed=True)
        self.add_link_name("4", "0", cost=4000, delay=0.2, bw=100000, directed=True)
        self.add_link_name("1", "4", cost=4000, delay=0.2, bw=200000, directed=True)
    ...
    def dijkstra_computed(self):
        # Demand 11->21 via ICA-JH waypoint node 1 (encap seg6 segs node_1)
        build_str = ""
        ...
        # Demand 12->22 via ICA-JH waypoint node 1 (encap seg6 segs node_1)
        build_str = ""
        nhlist = self.get_dijkstra_route_by_name("12", "1")
        for nh in nhlist:
            build_str += f" nexthop via {nh.nh} " + " encap seg6 mode inline segs {1} " + f" weight {int(100/len(nhlist))} "
        self.add_command("12", f"ip -6 route add {{22}} metric 1 table 1 src {{12}}  {build_str}")
        # Demand 13->23 via ICA-JH waypoint node 2 (encap seg6 segs node_2)
        build_str = ""
        ...
        # Demand 14->24 direct (ICA-JH: no waypoint, goes 14->0->4->24)
        build_str = ""
